...
namespace Ircis {
  bool Ircis::update() {
	...
	paths_[runner_id_].alive_from = step_number;
      ...
      runner_list_.emplace_back(runner_id_++, new_runner_info.position, grid_, log_, global_var_map_, new_runners_list_, new_runner_info.st, new_runner_info.var_map, new_runner_info.path);
